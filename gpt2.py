...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        B, T, C = x.size() # (batch_size, sequence_length, embedding dim)
        qkv = self.c_attn(x)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.attn(self.ln_1(x)) # Produce affinity between words
        x = x + self.mlp(self.ln_2(x)) # Think individually about the information gathered
        return x
...
while x.size(1) < max_length:  # max_length=30
    with torch.no_grad():
        logits = model(x)  # Ensure logits are of shape (B, T, vocab_size)
