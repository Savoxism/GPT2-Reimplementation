...
train_loader = DataLoaderLite(B=B, T=T) # adjust depending on the available memory
...
def get_lr(it):
    ...
    if it > max_steps:
        return min_lr
    # 3. in between, use cosine learning rate decay
...
# generate, right now x is (B, T) with B = 5, T = 6
